...
Design: Read postgres password via `1psa -p` default or `1psa -f` override field, then read `localhost_postgres_manifold` `host`/`port` and validate non-empty/valid values.
Tests:
